...
| `ai_copilot.py` | Optional: calls Claude Haiku to score trade quality → scales position size 0.5×–1.5× |
...
When `gambler_copilot.json` has `{"enabled": true}` and `ANTHROPIC_API_KEY` is set, each entry calls `ai_copilot.evaluate_trade()` which asks Claude Haiku for a confidence score (0–100). Position size is scaled by `0.5 + confidence/100`. Results cached 60s per signal.
